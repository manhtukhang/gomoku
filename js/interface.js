...
function showWinDialog(game){
    ...
    if(game.mode === 'hvh'){
        ...
        $("#game-won h4").html(who+' Thắng!');
        gameInfo.value=who+' thắng.'
        $("#win-content").html(who+' đã thắng. Không dám chơi nữa đúng ko?');
        $('#happy-outer').fadeIn(500);
    }else{
        if(game.getCurrentPlayer() instanceof HumanPlayer){
            $("#game-won h4").html('Bạn đã thắng!');
            $("#win-content").html('Bạn đã thắng, hên thôi. Chắc ko dám chơi nữa đâu!');
            gameInfo.value='Bạn đã thắng.'
            $('#sad-outer').fadeIn(500);
        }else{
            $("#game-won h4").html('Bạn đã thua.');
            $("#win-content").html('Lêu lêu. Thua máy rồi kìa. Chắc ko dám chơi nữa đâu!');
            gameInfo.value='Máy thắng.'
            $('#happy-outer').fadeIn(500);
        }
    }
}